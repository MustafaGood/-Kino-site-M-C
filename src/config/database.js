...
const connectDB = async () => {
  try {
    ...
    if (envConfig.useExternalMongoDB) {
      ...
      console.log(`📊 Produktions MongoDB URI: ${mongoURI.replace(/\/\/[^:]+:[^@]+@/, '//***:***@')}`);
      ...
      if (!mongod) {
        ...
        try {
          ...
          console.error('❌ Kunde inte starta MongoDB Memory Server:', memoryServerError.message);
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        console.log(`✅ MongoDB ansluten: ${conn.connection.host}:${conn.connection.port}`);
        console.log(`📊 Databas: ${conn.connection.name}`);
        console.log(`🔗 Anslutningsstatus: ${mongoose.connection.readyState === 1 ? 'Ansluten' : 'Ej ansluten'}`);
        ...
        retryCount++;
        console.error(`❌ Anslutningsförsök ${retryCount}/${maxRetries} misslyckades:`, connectError.message);
